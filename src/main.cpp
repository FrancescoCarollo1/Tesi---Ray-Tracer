...
const std::vector<std::string> scene_files = {
    "palla_triangolo.txt",
    "Città_Sfere.txt",
    "Foresta_cristallo.txt",
    "Galleria_specchi.txt",
    "Showroom_materiali.txt",
    "Test_veloce.txt"
};
const std::string base_path = "../libs/Ray-Tracing/prove_txt/";
...
int main(int, char **) {
    ...
    bool load_new_scene = true; 

    Vec3 camera_origin = {0,0,0};
    Vec3 camera_lookat = {0,0,-1};
    float cam_pitch = 0.0f;
    float cam_yaw = -90.0f;
    float f_fov = 40.0f;
    float f_aperture = 0.0f;
    float f_focus_dist = 10.0f;
...
// --- 4. MAIN LOOP ---
    while (!done)
    {
        ...
        if (load_new_scene) {
            ...
            if (scene != NULL) delete_scene(scene);
            
            // Allocazione nuova
            scene = create_empty_scene();
            
            std::string full_path = base_path + scene_files[current_scene_idx];
            printf("Loading: %s\n", full_path.c_str());
            
            if (read_scene(full_path.c_str(), scene) != 0) {
                printf("ERR: Could not load scene. Checking path...\n");
            }

            // Aggiorna variabili camera
            camera_origin = scene->camera.cam_lookfrom;
            camera_lookat = scene->camera.cam_lookat;
            camera_aspect_ratio = scene->camera.cam_aspect_ratio;
            f_fov = (float)scene->camera.cam_vfov;
            f_aperture = 0.0f; 
            
            Vec3 dist_v = sub(camera_lookat, camera_origin);
            f_focus_dist = (float)sqrt(dist_v.x*dist_v.x + dist_v.y*dist_v.y + dist_v.z*dist_v.z);
            
            Vec3 dir = normalize(dist_v);
            cam_pitch = asin(dir.y) * (180.0 / M_PI);
            cam_yaw = atan2(dir.z, dir.x) * (180.0 / M_PI);
        ...
        if (camera_changed) {
            frame_index = 1; gpu_frame_index = 1;
        }
        ...
        if (!use_gpu_render) {
            if (camera_changed || first_frame) {
                ...
                setup_camera(scene, camera_origin, camera_lookat, scene->camera.cam_vup, (double)f_fov, scene->camera.cam_aspect_ratio, (double)f_aperture, (double)f_focus_dist);
                first_frame = false;
            }
        ...
        // 6. RENDER GPU
        if (use_gpu_render) {
            if (gpu_frame_index <= max_samples) {
                accumFBO.bind();
                if (gpu_frame_index == 1) { glClearColor(0,0,0,1); glClear(GL_COLOR_BUFFER_BIT); }
                ...
                rayShader.setFloat("u_aperture", f_aperture);
                rayShader.setFloat("u_focus_dist", f_focus_dist);
                rayShader.setFloat("u_time", SDL_GetTicks() / 1000.0f);
                rayShader.setInt("u_frame_index", gpu_frame_index);

                int count = scene->num_spheres; if (count > 128) count = 128;
                ...
                int tri_count = scene->num_triangles; if (tri_count > 64) tri_count = 64;
                ...
                glDisable(GL_BLEND); accumFBO.unbind();
                gpu_frame_index++;
            }
            glBindFramebuffer(GL_READ_FRAMEBUFFER, accumFBO.FBO);
            glBindFramebuffer(GL_DRAW_FRAMEBUFFER, 0); 
            glViewport(0, 0, (int)io.DisplaySize.x, (int)io.DisplaySize.y); // Restore view
            glBlitFramebuffer(0, 0, 1280, 720, 0, 0, io.DisplaySize.x, io.DisplaySize.y, GL_COLOR_BUFFER_BIT, GL_LINEAR);
        ...
        ImGui::TextColored(ImVec4(1,1,0,1), "SCENE SELECTION");
        if(ImGui::BeginCombo("##files", scene_files[current_scene_idx].c_str())) {
            for(int n=0; n<scene_files.size(); n++) {
                if(ImGui::Selectable(scene_files[n].c_str(), current_scene_idx==n)) {
                    current_scene_idx = n; load_new_scene = true;
                }
            } ImGui::EndCombo();
        }
        ImGui::Separator();
        ...
        if (use_gpu_render) {
            ImGui::Text("GPU Samples: %d / %d", gpu_frame_index - 1, max_samples);
            if(gpu_frame_index > max_samples) ImGui::TextColored(ImVec4(0,1,0,1), "[DONE]");
            ImGui::SliderInt("Max Samples", &max_samples, 100, 5000);
        } else {
            ImGui::Text("CPU Samples: %d", frame_index);
        }

        if (ImGui::Checkbox("USE GPU RENDER", &use_gpu_render)) camera_changed = true;
        
        ImGui::Separator();
        if (ImGui::SliderFloat("FOV", &f_fov, 10.0f, 120.0f)) camera_changed = true;
        if (ImGui::SliderFloat("Aperture", &f_aperture, 0.0f, 2.0f)) camera_changed = true;
        if (ImGui::SliderFloat("Focus Dist", &f_focus_dist, 0.1f, 50.0f)) camera_changed = true;
